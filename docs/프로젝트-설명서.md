...
자기소개 한 번 → 관심사·강점이 부족할 때만 고정 질문 1회씩 → Q3
```
...
| LLM 호출 (온라인, 요청당) | 1 | 3질문 모드 2 (태거 + 설명) · 인터뷰 모드 ≤5 |
...
| **인터뷰 모드** | `python app.py --interview`. 자기소개 → `user_analysis` 분석 → 관심·강점만 우리 입력으로. LLM ≤3회 |
