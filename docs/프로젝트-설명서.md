...
| 역량 이름 (통제 어휘) + IS_A 계층 | 팀이 직접 관리 → `vocab.py` (`CANON`, `PARENT`) | 대표 표기 107개 / 부모 17개 |
...
| 파일 | 내용 | 기준 |
|---|---|---|
| `cases.json` | 3질문 모드 10건. 입력 3답 → 기대 전공(top-3)·직무명 조각(top-2) | 현재 9/10 (#7 은 기대값 재검토 — PM 하나뿐인 입력) |
...
### 현재 수치 (2026-09-16 새벽, `graph.json`)

| 항목 | 9/15 오전 | **지금** |
|---|---|---|
| 전공 (역량 1개 이상) | 63 / 105 | **59** / 105 (소프트 스킬로만 붙어 있던 전공이 빠짐) |
| 직무 (통제 어휘 역량 2개 이상) | 22 / 40 (CNS) | **84** / 172 (LG 계열사 9곳) · **신입 51** (LGES 34, CNS 9, LGD 4, LGU 2, HSAD 1, LGE 1) |
| Skill 노드 | 52 | **98** (부모 17 포함) |
| **양쪽 공통 Skill** | **8** | **22** — Algorithm, Chemical/Electrical/Materials/Mechanical Engineering, Cloud, Computer Vision, Data Analysis, Database, Deep Learning, Digital Twin, Machine Learning, Manufacturing, NLP, Network, OS, Optimization, Programming, Project Management, Robotics, Simulation, Statistics |
| IS_A 한 홉으로 이어지는 직무 역량 | — | **61** (AWS, Docker, Kubernetes, Java, Python, React, Spring Boot, LLM, RAG, CATIA, Simulink …) |
| 관계 종류 | 2 (DEVELOPS, REQUIRES) | **5** (+ IS_A, SUGGESTS, MATCHES/WANTS) |
| 평가셋 | 없음 | 3질문 **9/10** · 인터뷰 **2/2** · 확인 3건 통과 |
| LLM 호출 (온라인, 요청당) | 1 | 3질문 모드 2 (태거 + 설명) · 인터뷰 모드 ≤5 |

### 알려진 한계

1. **어휘 공백이 남아 있다.** 신입 직무 90건 중 51건만 그래프에 올랐다. 빠진 39건은 배터리 공정·설비·광고 등 IT 밖 어휘. `data/canon_candidates.txt`(548개, 빈도순)가 A 의 작업 목록.
...
7. 평가셋 #7(PM 하나뿐인 입력 → 경영학과 기대)은 기대값 자체를 팀이 재검토해야 한다. 근거대로는 건설환경도시공학부(건설 공정관리 3과목)가 맞다.
8. `graph_store._load()`/`_idf()` 는 프로세스 시작 시 1회만 읽는다. `app.py` 를 띄운 채 `graph.json` 을 다시 만들면 재시작.

### 다음 할 일 (2026-09-16 수요일 기준)

- [ ] PR 머지: `feat/screen-card-plus-paragraph`(카드+문단 — PR #4 작성자와 합의) → `feat/interview-mode`(인터뷰·온톨로지·태도)
- [ ] A: `canon_candidates.txt` 2회 이상 항목 재검토, `TRAIT_TO_SKILL`·`TRAIT_TO_SOFT` 승인, 평가셋 #7 기대값
...
- [ ] 발표 4·5장 수치 갱신: 전공 59 / 직무 84(신입 51) / 공통 Skill 22 / IS_A 61 / 관계 5종 / 평가셋 12건. "개수를 지정하면 지어낸다" 사례 4개(정확히 3개 · 4~8개 · 최대 5개 · 3문장 이상)
