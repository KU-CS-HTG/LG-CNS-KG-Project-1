...
def _job_lines(j: dict, indent: str) -> list[str]:
    """직무 한 건의 충족/계열/태도 줄. [직무](1위)와 [다음 후보]가 같은 형식을 쓴다 (9/16 팀 요청).

    9/17: 웹 <pre> 폭(LINE_WIDTH)을 넘는 줄은 항목 경계로만 나눈다(_wrap_items). "(✓ = 너의 강점 성향과 맞음)" 범례는
    여기서 빼고 render() 가 카드 끝에 한 번만 단다.
    공고 URL은 화면에서 뺀다(9/17 웹 피드백) — render_reverse() 는 그대로 보여준다(역방향 질의는 URL이 핵심 출처).
    """
    fam = j.get("family", {})
...
def render(r: dict) -> str:
    ...
    for s in r["subjects"]:
        skills = list(s["for"]) or ["역량 향상"]
        skills[-1] = f"{skills[-1]} 역량 향상"          # 마지막 역량에만 붙인다 — _wrap_items 가 폭까지 같이 계산해 준다
        L += _wrap_items(f"{s['subject']} → ", skills, IND, sep=", ")
    L.append("")
    ...
    if j:
        ...
        L += _job_lines(j, IND)
        # ── [다음 후보]  2·3위 — [직무]와 같은 줄 형식(9/16). 후보가 둘 이상이면 번호를 매기고 문단을 나눈다(9/17).
        #    신입 공고가 적어 1위가 몰리기 쉬운 것도 [다음 후보]를 두는 이유
        alts = r.get("jobs", [])[1:3]
        if alts:
            L.append("")
            L.append("[다음 후보]")
            for i, alt in enumerate(alts, 1):
                if i > 1:
                    L.append("")                                           # 후보 사이 문단 나누기
                prefix = f"{i}. " if len(alts) > 1 else ""
                L.append(f"{IND}{prefix}{alt['company']} {alt['role']} ({alt['career_type']})")
...
단, 1단락에는 "LG 직무와 연결되는 전공 N개를 모두 살펴봤고, 학생의 역량과 관련된 전공 M개 중 가장 잘 어울리는 전공"
이라는 취지의 문장을 **반드시 한 번** 친절한 말로 넣는다 — 숫자 N, M 은 [근거]의 "전체 전공 수 / 이어진 전공 수" 값을
그대로 쓴다. 이 문장에는 특정 회사·직무 이름을 넣지 않는다 — "LG 직무"라고만 쓴다(구체적인 회사·직무 이름은
3단락에서 자연스럽게 언급한다). 처음부터 그 직무 하나만 보고 전공을 골랐다는 인상을 주면 안 된다.
같은 문장(또는 바로 다음 문장)에서 **왜 가장 잘 어울리는지**를 [근거]의 "[학생 역량이 이 전공에 이어진 방식]" 항목으로
...
(그 목록은 과목 3개만 보여주므로 역량이 빠져 있을 수 있다). 미연결이 0이면 말하지 않는다. 계열로 이어진 역량이
있을 때만 "~계열로 이어져요"라고 쓰고, 미연결 역량을 계열로 이어졌다고 쓰지 않는다.
